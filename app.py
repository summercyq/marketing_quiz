...
if mode == "題庫編輯":
    ...
    with st.expander("🛠️ 管理者登入"):
        admin_pwd = st.text_input("🔐 請輸入管理密碼", type="password", key="admin_pwd_in_button")
        if admin_pwd == EDIT_PASSWORD:
            admin_action = st.radio("選擇管理功能", ["題庫編輯", "錯題紀錄管理", "下載紀錄"], key="admin_action_button")
            if admin_action == "題庫編輯":
                ...
                clear_mode = st.radio("清除模式", ["單一使用者", "全部使用者"], key="admin_clear_mode")
                if clear_mode == "單一使用者":
                ...
                target = st.radio("📥 選擇下載檔案", ["錯題紀錄", "答題統計"], key="admin_download")
                if target == "錯題紀錄":
                    if os.path.exists(WRONG_LOG):
...
# === 管理者整合模式：題庫編輯 + 錯題清除 + 記錄下載 ===
if mode == "管理者登入":
    admin_pwd = st.text_input("🔐 管理者密碼", type="password", key="admin_pwd")
    if admin_pwd == EDIT_PASSWORD:
        action = st.radio("管理功能選擇", ["題庫編輯", "錯題紀錄管理", "下載紀錄"])
        ...
        if action == "題庫編輯":
            ...
            target = st.radio("📥 請選擇下載檔案", ["錯題紀錄", "答題統計"], key="admin_download")
            if target == "錯題紀錄":
                if os.path.exists(WRONG_LOG):
                    with open(WRONG_LOG, "rb") as f:
                        st.download_button("📎 下載錯題紀錄.csv", data=f, file_name="錯題紀錄.csv")
